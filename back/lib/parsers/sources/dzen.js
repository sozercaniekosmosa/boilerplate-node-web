...
import {getHashCyrb53} from "../utils/utils.js";
...
const getID = (url) => {
    ...
    return getHashCyrb53(_url.toString());
}
...
function getTextContent(doc) { //получить текст новости
    return doc.dataSource.news.story.summarization.items.map(it => it.text).join(' ');
}

function isExistID(id, data) { //проверить id существует в БД?
    throw 'Необходимо инициировать метод isExistID'
}

function onStoreToDB(id, data) { //сохранить в БД
    throw 'Необходимо инициировать метод onStoreToDB'
}
