...
export class TelegramChannelBot {
    ...
    async #restoreState() {
        const {scheduledMessages, publishedMessages} = this.dbTB.getByID('tb') ?? {
            scheduledMessages: {},
            publishedMessages: {}
        };
    ...
    async #sendStorageMessage(chatID, threadID, text, arrImg) {
        ...
        try {
            ...
            if (arrImg.length > 1) {
                ...
                arrResponse = await this.bot.sendMediaGroup(chatID, media, {
                    message_thread_id: threadID,
                    text,
                    parse_mode: 'HTML'
                })
            } else {
                arrResponse = [await this.bot.sendPhoto(chatID, arrImg[0], {
                    message_thread_id: threadID,
                    caption: text,
                    parse_mode: 'HTML'
                })];
    ...
    async editMessage(channelID, messageTaskID, newText, arrImg) {
        ...
        if (arrImg.length) {
            ...
            for (let i = 0; i < arrImg.length; i++) {
                ...
                await this.bot.editMessageMedia({type: 'photo', media: img}, {
                    chat_id: channelID,
                    message_id: arrMessage[i]
                });
            }
        }
...
// new TelegramChannelBot(BOT_TOKEN, TG_GROUP_STORAGE_ID, TG_GROUP_STORAGE_THREAD_ID, global.dbTB);